...
describe("DAO Contract Testing Flow", function () {
  it("Proposal to Executed Flow", async function () {
    ...
    const Bank = await ethers.deployContract("Bank", [TimeLock.target]);
    ...
    const transferCalldata = Bank.interface.encodeFunctionData("transfer", [
      ethers.parseEther("1"), // Amount to transfer (1 ETH)
      deployer.address, // Recipient's address
    ]);
    ...
    proposeTx = await MyGovernor.propose(
      ...
      [0],
      [transferCalldata],
      "Proposal #1: Transfer Amount"
    );
    await proposeTx.wait();
    const filter = MyGovernor.filters.ProposalCreated();

    const events = await MyGovernor.queryFilter(
      filter,
    ...
    const descriptionHash = ethers.id("Proposal #1: Transfer Amount");
    ...
    console.log(await Bank.Certificates(101));
  });
});
